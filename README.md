MineCube is a custom Minecraft Classic and MineCube client written in C that works on Windows, macOS, Linux, Android, iOS, FreeBSD, NetBSD, OpenBSD, Solaris, Haiku, and in a web browser.<br>
...
#### Instructions
Initially, you will need to run MineCube.exe to download the required assets from minecraft.net. 
Just click 'OK' to the dialog menu that appears when you start the launcher.

**Singleplayer**
Run MineCube.exe, then click Singleplayer at the main menu.

**Multiplayer**
Run MineCube.exe. You can connect to LAN/locally hosted servers, ~~minecraft.net servers,~~ and MineCube.net servers through the launcher.
...
##### Using Visual Studio
Open MineCube.sln *(File -> Open -> Project/Solution)* and compile it *(Build -> Build Solution)*.
...
3. Enter `cl.exe *.c /link user32.lib gdi32.lib crypt32.lib ws2_32.lib wininet.lib winmm.lib dbghelp.lib shell32.lib comdlg32.lib /out:MineCube.exe`
...
4. Enter `gcc *.c -o MineCube.exe -mwindows -lwinmm -limagehlp`
...
```gcc *.c -o MineCube -rdynamic -lm -lpthread -lX11 -lXi -lGL -ldl```

##### Cross compiling for Windows (32 bit):

```i686-w64-mingw32-gcc *.c -o MineCube.exe -mwindows -lwinmm -limagehlp```

##### Cross compiling for Windows (64 bit):

```x86_64-w64-mingw32-gcc *.c -o MineCube.exe -mwindows -lwinmm -limagehlp```

##### Raspberry Pi
Although the regular linux compiliation flags will work fine, to take full advantage of the hardware:

```gcc *.c -o MineCube -DCC_BUILD_RPI -rdynamic -lm -lpthread -lX11 -lXi -lEGL -lGLESv2 -ldl```

## Compiling - macOS

##### Using gcc/clang (32 bit)

```cc *.c -o MineCube -framework Carbon -framework AGL -framework OpenGL -framework IOKit```

##### Using gcc/clang (64 bit)

```cc *.c interop_cocoa.m -o MineCube -framework Cocoa -framework OpenGL -framework IOKit -lobjc```
...
```cc *.c -o MineCube -I /usr/local/include -L /usr/local/lib -lm -lpthread -lX11 -lXi -lGL -lexecinfo```

#### OpenBSD

Install libexecinfo, curl and openal package if needed

```cc *.c -o MineCube -I /usr/X11R6/include -I /usr/local/include -L /usr/X11R6/lib -L /usr/local/lib -lm -lpthread -lX11 -lXi -lGL -lexecinfo```

#### NetBSD

Install libexecinfo, curl and openal-soft package if needed

```cc *.c -o MineCube -I /usr/X11R7/include -I /usr/pkg/include -L /usr/X11R7/lib -L /usr/pkg/lib  -lpthread -lX11 -lXi -lGL -lexecinfo```
...
```gcc *.c -o MineCube -lm -lsocket -lX11 -lXi -lGL```

#### Haiku

Install openal_devel and libexecinfo_devel package if needed

```cc *.c Window_Haiku.cpp -o MineCube -lm -lexecinfo -lGL -lnetwork -lstdc++ -lbe -lgame -ltracker```

#### SerenityOS

Install SDL2 port if needed

```cc *.c -o MineCube -lgl -lSDL2```
...
## Sound Credits
MineCube uses sounds from [Freesound.org](https://freesound.org)<br>
Full credits are listed in [doc/sound-credits.md](doc/sound-credits.md)

## NOTE

That is a modified Classicube client!
